...
func isMgmtVrfEnabled(inParams XfmrParams) bool {
	...
	if err == nil {
		...
		if enabled_status == "true" {
			log.Infof("Called Mgmt_vrf",enabled_status)
			return true
		}
	}
	return false
...
var YangToDb_sflow_collector_xfmr SubTreeXfmrYangToDb = func(inParams XfmrParams) (map[string]map[string]db.Value, error) {
	...
	pathInfo := NewPathInfo(inParams.uri)
	if((pathInfo.Var("network-instance") == "mgmt") && !(isMgmtVrfEnabled(inParams))) {
		oper_err := errors.New("Management vrf is not enabled")
		log.Errorf("Management Vrf is not enabled")
		return res_map,oper_err
	}
