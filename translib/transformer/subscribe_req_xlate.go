...
func (pathXltr *subscribePathXlator) setTrgtYgXpathInfo() error {
	...
	for ygXpathInfo != nil && len(ygXpathInfo.xfmrKey) == 0 {
		...
		if ygXpathInfoTmp, ok := xYangSpecMap[ygPathTmp]; !ok || ygXpathInfoTmp == nil {
			log.Warning(pathXltr.subReq.reqLogId, "xYangSpecMap does not have the yangXpathInfo for the path:", ygPathTmp)
...
func GetSubscribeReqXlator(subReqId interface{}, reqUri string, reqMode NotificationType, dbs [db.MaxDB]*db.DB, txCache interface{}) (*subscribeReqXlator, error) {
	reqIdLogStr := "subReq Id:[" + fmt.Sprintf("%v", subReqId) + "] : "
	log.Infof("%v GetSubscribeReqXlator: for the reqUri: %v; reqMode: %v; txCache: %v", reqIdLogStr, reqUri, reqMode, txCache)
	...
	if subReq.ygPath, _, err = XfmrRemoveXPATHPredicates(reqUri); err != nil {
		log.Warning(subReq.reqLogId, "Got error from the XfmrRemoveXPATHPredicates function: ", err)
		return nil, err
	}

	if subReq.gPath, err = ygot.StringToPath(reqUri, ygot.StructuredPath); err != nil {
		log.Warning(subReq.reqLogId, "Error in converting the URI into GNMI path for the URI: ", reqUri)
...
func (pathXltr *subscribePathXlator) handleYangToDbKeyXfmr() (string, error) {
	...
	if len(pathXltr.keyXfmrYgXpathInfo.xfmrKey) > 0 {
		...
		if err != nil {
			log.Warning(pathXltr.subReq.reqLogId, "Error: unMarshallYgotObj error: ", err)
			return "", err
		}
